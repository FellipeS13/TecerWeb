...
app.post('/loginApp', (req, res) => {
  ...
  db.query(query, [cpf, senha], (error, results) => {
    if (error) {
      ...
      if (results.length > 0) {
        // Login bem-sucedido
        res.status(200).json({ loginID: results[0].id, message: 'Login bem-sucedido' });
      } else {
        // Login inválido
...
app.post('/obterDocumentosApp', (req, res) => {
  // Consulta ao banco de dados para obter a lista de documentos
  const clienteID = req.body.clienteID;
  const categDOC = req.body.categoriaDOC;
  const documentosQuery = 'SELECT d.id, c.nome, d.arquivo AS documentLink, cat.nome AS nome_categoria, d.nome AS name FROM documentos d JOIN cliente c ON d.cliente_id = c.id JOIN categorias cat ON d.categoria_id = cat.id WHERE d.cliente_id = ? AND cat.id = ?;'
  db.query(documentosQuery, [clienteID, categDOC], (err, results) => {
    if (err) {
      console.error(err);
      res.status(500).json({ message: 'Erro no servidor' });
    } else {
